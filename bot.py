...
def get_photo_id(message):
    ...
    bot.send_message(my_chat_id, file_id)

# Обработчик команды /start
@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(my_chat_id, "Привет! Я телеграм-бот, который отправляет картинки каждые полчаса с понедельника по пятницу с 11 утра до полуночи. Нажмите кнопку 'Готово', чтобы остановить отправку на текущий день, или 'Пауза на 2 часа', чтобы сделать паузу в отправке.")
...
def stop_sending(message):
    global is_paused
    is_paused = True
    bot.send_message(my_chat_id, "Отправка картинок остановлена на текущий день.", reply_markup=markup_clear)

# Обработчик кнопки "Пауза на 2 часа"
@bot.message_handler(func=lambda message: message.text == "Напомни через 2 часа")
def pause_sending(message):
    global is_paused
    is_paused = True
    bot.send_message(my_chat_id, "Отправка картинок приостановлена на следующие два часа.", reply_markup=markup_clear)
    time.sleep(pause_duration)
    is_paused = False
    bot.send_message(my_chat_id, "Пауза завершена. Продолжаем отправку картинок.", reply_markup=markup_clear)
...
def send_image():
    while True:
        if not is_paused:
            now = datetime.now()
            week_int = math.floor((now.date() - date(2024, 4, 2)).days / 7) % 8
            week = str(week_int + 1)
            ...
            if now.weekday() in schedule_days and start_hour <= now.hour < end_hour:
                ...
                try:
                    ...
                    print(e)
                    bot.send_message(my_chat_id, e, reply_markup=markup_clear)
                pass
